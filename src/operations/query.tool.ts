...
When NOT to use bc_query: do not use for UI-driven flows (navigating pages, clicking buttons, filling forms — use bc_open_page + bc_execute_action for those). Do not use bc_query for posting, writing, or triggering BC business logic — OData reads are read-only; use bc_write_data and bc_execute_action for mutations. Do not use for custom/extension entities not in the Standard API v2.0 — those require the UI WebSocket tools. Auth: on-prem NavUserPassword uses HTTP Basic; BC Online (SaaS) still needs Entra API credentials (BC_CLIENT_ID device code, BC_CLIENT_SECRET S2S, or BC_ACCESS_TOKEN) and returns OAUTH_NOT_CONFIGURED without them. bc_query does not need a /csh WebSocket session and does not open the SaaS sign-in window.`,
