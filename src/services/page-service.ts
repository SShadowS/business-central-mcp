...
import type {
  BCEvent, OpenFormInteraction, CloseFormInteraction, InvokeActionInteraction,
} from '../protocol/types.js';
import { buildFormTree } from '../protocol/form-tree-builder.js';
import { isGroupNode, isLogicalFormNode } from '../protocol/form-node.js';
import type { Logger } from '../core/logger.js';
import type { SectionKind } from '../protocol/section-resolver.js';
import type { WizardState } from '../protocol/types.js';
import { ChildFormHydrationStrategy, DEFAULT_AUTO_LOAD_SECTIONS } from './strategies/child-form-hydration.js';
...
// DEFAULT_AUTO_LOAD_SECTIONS now lives with the ChildFormHydrationStrategy;
// re-exported here for existing importers.
export { DEFAULT_AUTO_LOAD_SECTIONS };
...
export class PageService {
  ...
  constructor(
    ...
    this.childFormHydration = new ChildFormHydrationStrategy(session, repo, logger, this.autoLoadSections);
  }
  ...
  async openPage(pageId: string, options?: { bookmark?: string; tenantId?: string }): Promise<Result<PageContext, ProtocolError>> {
    ...
    // Discover child forms embedded in the root form's control tree (fhc -> lf
    // nodes) and load their lines/factbox data. Shared with the drill-down and
    // action-opened-page paths so those targets get their lines section too.
    await this.childFormHydration.hydrate(pageContextId, events);
